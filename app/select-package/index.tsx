...
const SelectPackage = () => {
    ...
    return (
        <SafeAreaView className={"flex flex-col items-center justify-center w-full h-full p-2"}>
            <View className={"w-full mb-2 p-2"}>
                <Searchbar
                ...
                <View className={"flex flex-row items-center justify-between w-full my-2"}>
                    <Surface mode="flat" style={{backgroundColor: "rgba(241,234,234,0.71)", borderRadius: 12, paddingVertical: 4, paddingHorizontal: 8}} className={"flex flex-row items-center justify-end"}>
                        <Switch
                            value={includeSystemApps}
                            onValueChange={handleIncludeSystemAppsChange}
                        />
                        <Pressable onPress={toggleIncludeSystemApps}>
                            <Text className={"text-xs text-gray-500"}>Include system apps</Text>
                        </Pressable>
                    </Surface>
                </View>
            </View>
                ...
                <FlatList
                    ...
                    renderItem={ (i: any) =>  {
                        return (
                            ...
                            >
                                <Surface style={{padding: 8, borderRadius: 12}} className={"flex flex-row items-center justify-start"}>
                                    <Avatar.Image size={55} source={{uri: `data:image/png;base64,${i.item.icon}`}} />
